...
func SearchBiometries(c *fiber.Ctx) error {
	db := database.DB

	typeBiometrie := c.Query("type_biometrie")
	qualite := c.Query("qualite")
	verifie := c.Query("verifie")
	...
	if qualite != "" {
		query = query.Where("qualite_donnee = ?", qualite)
	}
	if verifie != "" {
		isVerified := verifie == "true"
		query = query.Where("verifie = ?", isVerified)
	...
	if dateFrom != "" {
		query = query.Where("date_capture >= ?", dateFrom)
	}
	if dateTo != "" {
		query = query.Where("date_capture <= ?", dateTo)
	}
	if minConfidence != "" {
		if confidence, err := strconv.ParseFloat(minConfidence, 64); err == nil {
			query = query.Where("score_confiance >= ?", confidence)
		}
	}
	...
	if err != nil {
		return c.Status(500).JSON(fiber.Map{
			"status":  "error",
			"message": "Failed to search biometric data",
			"error":   err.Error(),
		})
	}
	...
	return c.JSON(fiber.Map{
		"status":  "success",
		"message": "Search results for biometric data",
		"data":    biometries,
	})
}
